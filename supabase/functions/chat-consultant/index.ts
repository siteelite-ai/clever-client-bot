...
- ⚡ запрос выглядит как ПОЛНОЕ НАЗВАНИЕ КАРТОЧКИ из каталога: тип товара + НЕСКОЛЬКО точных уникальных параметров, которые однозначно идентифицируют конкретную модель (например: точные габариты формата `AxBxC мм`, код защиты `IP##`, точное число «модулей/полюсов/жил», фиксированная маркировка). Даже БЕЗ бренда — если параметров достаточно, чтобы это читалось как название товара, а не фильтр категории.

Если это КОНКРЕТНЫЙ товар:
  → has_product_name = true
  → product_name = ПОЛНОЕ название как поисковый запрос (бренд + серия + ключевые параметры + артикул, без разговорных слов и без вопросительных оборотов «есть в наличии», «сколько стоит»)
...
- "Щит для автом. выключателей на 2-4 модуля 75*124*57мм IP20 есть в наличии?" → product_name="Щит для автоматических выключателей на 2-4 модуля 75*124*57мм IP20", product_category="щит" (тип + габариты AxBxC + IP-код = полное название карточки даже без бренда)
...
- "настольная лампа" → category="лампа", modifiers=["настольная"]
- "напольный светильник" → category="светильник", modifiers=["напольный"]
- "потолочная люстра" → category="люстра", modifiers=["потолочная"]
- "уличный фонарь" → category="фонарь", modifiers=["уличный"]
- "светодиодная лента" → category="лента", modifiers=["светодиодная"]
- "щит на 12 модулей" → category="щит", modifiers=["12 модулей"] (один параметр без габаритов/IP — это ещё фильтр, не название карточки)

Ключевое отличие: БРЕНД+ТИП или ТИП+СЕРИЯ+ПАРАМЕТРЫ+АРТИКУЛ или ТИП+комбинация уникальных параметров (габариты AxBxC + IP-код / точная маркировка) → конкретный товар. Тип + 1-2 общих характеристики без бренда/уникальной комбинации → категория.

ЛИНГВИСТИЧЕСКОЕ ПРАВИЛО (применяй ВСЕГДА при разборе именной группы товара):
- product_category = ГЛАВНОЕ существительное именной группы (что это за предмет по сути) — одно слово или устойчивый составной термин.
- search_modifiers ОБЯЗАН содержать ВСЕ значимые слова исходного запроса, КРОМЕ самого product_category. Алгоритм: возьми текст запроса → удали слово/слова, попавшие в product_category → всё остальное, что несёт смысл (т.е. сужает или уточняет товар), помести в search_modifiers как отдельные элементы. Удаляй ТОЛЬКО разговорные обёртки, не несущие критериев товара ("у тебя есть", "подскажи", "пожалуйста", "мне нужно", "хочу", "посоветуй", вопросительные знаки).
- К значимым относятся (неисчерпывающе): согласованные прилагательные и причастия любого типа, ВСЕ предложные группы ("с …", "без …", "для …", "под …", "на …", "в …", "из …"), числительные, единицы измерения, маркировки, артикулы, бренды, цвета, материалы, любые параметры. Если сомневаешься — включай.
- НИКОГДА не склеивай прилагательное+существительное в одну category ("настольная лампа" → category="лампа", modifiers=["настольная"]). Исключение: устойчивый составной термин, без второго слова меняющий класс товара ("автоматический выключатель" → category="автоматический выключатель"). В сомнительных случаях предпочитай разделение.
- ПРОВЕРКА (выполни ПЕРЕД ответом): мысленно склей product_category и все элементы search_modifiers через пробел — должны вернуться все смысловые слова исходного запроса. Если что-то потерялось — добавь в search_modifiers.
...
4. product_category (string|null): БАЗОВЫЙ тип товара — максимально общее слово/устойчивый термин товарной группы. НЕ включай сюда никакие уточнения — они идут в search_modifiers.
...
6. search_modifiers (string[]): применяй правило «всё значимое из запроса кроме product_category» из лингвистического блока выше. Не фильтруй по типу характеристики — система сама сопоставит модификаторы с реальными опциями товаров. Если запрос состоит ТОЛЬКО из категории — пустой массив.

7. critical_modifiers (string[]): ПОДМНОЖЕСТВО search_modifiers, которые пользователь требует КАТЕГОРИЧНО (без них товар не подходит). Определяй по тону:
- Просто перечисленные характеристики — ВСЕ критичные.
- Смягчающие слова рядом ("примерно", "около", "желательно", "можно", "лучше", "хотелось бы") — соответствующий модификатор НЕ критичный.
- Нет модификаторов — пустой массив.

КЛЮЧЕВОЙ ПРИНЦИП: category = базовый тип товара для широкого текстового поиска. ВСЁ остальное значимое из запроса → search_modifiers, без потерь. critical_modifiers говорит системе, какие фильтры НЕЛЬЗЯ ослаблять при fallback.
