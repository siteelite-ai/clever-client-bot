...
function buildUserMessageNoResults(
  ...
  switch (scenario) {
    ...
    case "error":
      lines.push(
        "Каталог временно недоступен (технический сбой на стороне поставщика данных). НЕ говорите «ничего не нашлось» — это неправда: вы просто не смогли проверить. Действуйте так:",
        "1) Одной короткой честной фразой признайте, что прямо сейчас не можете показать товары из-за временной недоступности каталога.",
        "2) Помогите клиенту словом по сути его запроса: 2-4 коротких пункта — что важно учесть при выборе, какие ключевые характеристики стоит уточнить (мощность, тип цоколя, IP, сечение и т.п. — релевантное запросу). Если запрос слишком общий — задайте 1 уточняющий вопрос про задачу/применение.",
        "3) Контактную карточку менеджера и предложение вернуться чуть позже добавит система — НЕ дублируйте.",
        "Никаких выдуманных артикулов, цен, названий или ссылок. Только экспертная консультация словом.",
      );
      break;
    default:
      break;
