...
const INTRO_MODEL = "google/gemini-2.5-flash";
...
- АНТИ-ГАЛЛЮЦИНАЦИЯ (строго): называть конкретное значение характеристики (сечение, мм², ампераж, материал проводника, количество жил, диаметр, температуру, IP-класс, негорючесть и т.п.) можно ТОЛЬКО при выполнении ВСЕХ условий:
    (а) этот параметр ЯВНО присутствует в списке характеристик у КАЖДОГО из подобранных товаров (если хоть у одного товара параметра нет — считается «разное», упоминать НЕЛЬЗЯ);
    (б) у всех товаров значение этого параметра одинаковое (точное совпадение строки).
  Если условие (а) или (б) не выполнено — НЕ упоминай параметр вообще ИЛИ говори максимально обобщённо («с подходящим сечением», «нужного типа») без конкретики.
  То же правило для качественных свойств («негорючий», «медный», «бронированный», «экранированный»): называть можно, только если у КАЖДОГО товара соответствующая опция явно присутствует со значением «Да» / нужным материалом.
- Лучше короче и без характеристики, чем с неверной характеристикой.
