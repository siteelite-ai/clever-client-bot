...
export const TOOL_SCHEMAS = [
  {
    ...
    function: {
      ...
      description:
        "Получить схему категории каталога: pagetitle, фасеты (ключи, названия, единицы, фактические значения). ВСЕГДА вызывай первым для любого подбора товара по типу/характеристикам — без этого ты не знаешь реальных ключей и значений фасетов.",
      parameters: {
        type: "object",
        properties: {
...
- БЕЗ мета-фраз про процесс: «сейчас поищу», «попробую через…», «подбираю силовой кабель мощностью…».
...
- discover_category вернул category_not_found → попробуй синоним noun → не вышло → jargon_recover_catalog или search_catalog(by_query).
...
- 2 пустых поиска подряд / вне профиля → escalate_to_manager.
...
3. Если в первом пузыре назвал число (сечение, мощность, диаметр) — финальные карточки ДОЛЖНЫ содержать только товары с этим числом. Иначе клиент видит противоречие. Не уверен в наличии значения — формулируй мягко без числа.
4. propose_clarification + render_products в одном ходе = баг.
5. До исчерпания двухшагового паттерна + хотя бы одной резервной ветки запрещено говорить «нет в наличии», «сняли с производства», «не производят».
