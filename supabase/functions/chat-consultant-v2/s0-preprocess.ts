...
// `\b` в JS Unicode-режиме опирается на ASCII word chars и неправильно
// работает на границе кириллической буквы и пунктуации. Поэтому используем
// negative lookahead на «не буква и не цифра» — он корректен в `u`-режиме.
const GREETING_HEAD_RE =
  /^\s*(здравствуй(те)?|приветствую|привет|добр(ый|ое|ая)\s+(день|вечер|утро)|доброго\s+времени(\s+суток)?|хэллоу|хеллоу|хелло|хай|hello|hey|hi|салем|сәлем|саламатсыз\s*ба)(?![\p{L}\p{N}])[\s,.!?–—\-:;)(👋🙂😊🤝]*/iu;
