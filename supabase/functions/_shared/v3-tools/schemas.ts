...
3. **Сортировка и валидация рендера**: при «подешевле» сортируй ASC, при «дороже» DESC. В render_products НЕ передавай карточки, нарушающие ценовое направление (например цены выше якоря при «подешевле»).
...
Если у запроса нет якоря (просто «самый дешёвый <тип>») — search_catalog с сортировкой ASC по цене, ОБЯЗАТЕЛЬНО min_price=1 (товары с price=0 запрещены к показу).
