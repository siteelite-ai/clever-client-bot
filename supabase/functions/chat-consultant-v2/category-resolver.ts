...
//
// ──────────────────────────────────────────────────────────────────────────
// ARCHITECTURAL NOTE (F.4.2 — Stage F.4 architect review, 2026-04-29).
// ──────────────────────────────────────────────────────────────────────────
// Category resolution в V2 выполняется ОДНИМ LLM-вызовом по ПЛОСКОМУ списку
// всех pagetitle (см. `buildUserPrompt` ниже: `categories.map((c, i) => …)`).
//
// Этот модуль СОЗНАТЕЛЬНО НЕ реализует «category tree walk» / рекурсивный
// обход иерархии категорий. Решение нормативно по следующим причинам:
//
//   1. Spec §0 (data-agnostic): tree walk потребовал бы знания структуры
//      дерева 220volt, что нарушает контракт «zero hardcoded values».
//   2. Core Memory «Bot NEVER self-narrows funnel»: автоматический спуск
//      по дереву = self-narrowing. Неоднозначность разрешается через
//      `status='ambiguous'` + слот `category_disambiguation` (spec §9.2),
//      это user-driven процесс, а не bot-driven.
//   3. Latency: один LLM-вызов по плоскому списку ≈ один round-trip.
//      Tree walk = N round-trips (по уровням), несовместимо с SSE-потоком.
//
// Если в будущем возникнет соблазн добавить «умный» обход дерева — сначала
// перечитать §0, §9.2 и эту заметку. Любое такое изменение требует
// архитектурного ревью и обновления спеки, а не патча на месте.
