...
- Если у товара есть `related_sku` (поле API `soputstvuyuschiy`) — предложить контекстно одной строкой: «К этой розетке подходят рамки: [ссылка]». Кросс-сейл также фильтруется по `price > 0` (см. §9C.3).
