...
function formatProductsForAI(products: Product[]): string {
  if (products.length === 0) {
    return ''; // Пустая строка — бот сам задаст уточняющие вопросы
  }
...
ВАЖНО: Ссылки в данных уже готовы! Просто скопируй их как есть в формате [Название](URL). НЕ МЕНЯЙ URL!`;
      ...
      // Товары не найдены — задаём уточняющие вопросы вместо "не найдено"
      productInstructions = `
ПО ЗАПРОСУ "${userMessage}" ТОВАРЫ НЕ НАЙДЕНЫ В КАТАЛОГЕ.

⚠️ КРИТИЧЕСКИ ВАЖНО — НЕ ПИШИ "товары не найдены", "ничего не найдено", "не удалось найти"!

ВМЕСТО ЭТОГО ЗАДАЙ УТОЧНЯЮЩИЕ ВОПРОСЫ:
1. Уточни, что именно ищет клиент (для какой задачи, какой тип инструмента)
2. Спроси про бренд, мощность, ценовой диапазон
3. Предложи похожие категории товаров из нашего каталога

ПРИМЕРЫ ОТВЕТОВ:
- "Подскажите, для каких работ вам нужен инструмент? Это поможет подобрать оптимальный вариант."
- "Уточните, пожалуйста: вас интересует профессиональный инструмент или для домашнего использования?"
- "Какой бюджет вы рассматриваете? Есть ли предпочтения по бренду?"

ЕСЛИ ЗАПРОС СОВСЕМ НЕ ПО ТЕМЕ (кино, еда, погода и т.д.):
- Вежливо объясни, что ты консультант магазина электроинструментов
- НЕ пиши "к сожалению" или "не могу помочь"
- Предложи: "Зато я отлично разбираюсь в инструментах! Может, подскажу что-нибудь для дома или дачи?"
- Предложи конкретные популярные категории: дрели, шуруповёрты, болгарки, газонокосилки`;
    }
