...
- ❌ **Авто-сужение воронки** ботом: применение `?min_price=`/`?max_price=`, не названных пользователем; добавление в `slot.applied_filters` значения фасета (бренд, тип, цвет, мощность и т.п.), не упомянутого в реплике пользователя; молчаливое усечение выдачи для сортировок (например, «top-N из первых 50» при `pagination.total > full_scan_limit`). Воронка сужается **только** через явные ответы пользователя на clarification-вопросы. Любое нарушение — defect категории `auto_narrowing` (метрика `auto_narrowing_attempts_total` обязана быть строго 0). См. §9.7, §9.8, §9C.1.
- ❌ Clarification-вопрос, в котором предложено хотя бы одно значение, **отсутствующее в текущей воронке** (т.е. с `count = 0` среди уже отфильтрованных товаров). Любое срабатывание = defect `clarification_zero_result` (метрика обязана быть строго 0). См. §9.8.
...
### 9.7 Сортировка по цене — Scan-or-Clarify

API игнорирует `?sort=` (дефект **B-API-003**, см. §9C.1). Сортировка только локальная. Главный архитектурный принцип: **бот сам не сужает воронку** — никакого `min_price=<медиана>`, никакого молчаливого «top-N из первых 50», никакого выбора бренда/типа за пользователя. Воронка сужается **только** через ответы пользователя на clarification-вопросы (§9.8).

**Алгоритм** для `slot.sort ∈ { 'price_asc', 'price_desc' }` (применяется после Category Resolver и Facet Matcher, когда уже учтены все фильтры из реплики пользователя):

1. Запросить первую страницу `?category=…&options[…]=…&per_page=50&page=1` и взять `pagination.total`.
2. **Если `total ≤ app_settings.price_sort.full_scan_limit` (default 300)** — выполнить **полный честный скан**:
   - параллельно подгрузить все оставшиеся страницы (concurrency ≤ `price_sort.parallel_request_concurrency`, default 4; всего ≤ `price_sort.max_pages_parallel`, default 6);
   - локально отбросить `price === 0` (см. §9C.3);
   - локально отсортировать по цене и отдать топ-N (≤ 7, см. §9.6);
   - метрика `price_sort_full_scan_total++`, `price_sort_pages_fetched` фиксирует фактическое число страниц.
3. **Если `total > full_scan_limit`** — **clarification-mode**:
   - **товары и цена не отдаются**;
   - вызвать §9.8 «Clarification on funnel» — построить уточняющий вопрос по «делящему» фасету **из текущей воронки** (значения берутся из `product.options[]` уже скачанных товаров, никогда из `/categories/options` без сверки);
   - метрика `price_sort_clarification_required_total++`, `funnel_narrowing_rounds++`;
   - после ответа пользователя — повторно с шага 1 на сужённой воронке.
4. **Если после нескольких раундов уточнений `total` всё ещё > `full_scan_limit` и больше «делящих» фасетов нет** (§9.8 не нашёл фасет с ≥2 значениями `count ≥ 1`) — выполнить полный честный скан **первых** `full_scan_limit` товаров с явным предупреждением Composer'а: «Под ваши критерии {total} позиций — показал самые дешёвые из 300 точно отсканированных. Чтобы найти точный минимум по всему ассортименту — сузьте по бренду или типу.» Метрика `price_sort_partial_scan_with_warning_total++`.

**Запрещено** (см. §4.5):
- использовать `?min_price=`/`?max_price=`, **не названные** пользователем явно (например, «лампы до 5000 ₸» — можно; «дешёвые лампы» — **нельзя** автоматически ставить max_price=медиана);
- молча отдавать «top-N из первых 50» при `total > full_scan_limit` — это даёт некорректный «минимум среди первых 50 случайных»;
- выбирать значение фасета (бренд/тип/цвет) за пользователя.

**`?min_price=`/`?max_price=`** работают на стороне API и должны использоваться **только когда пользователь явно назвал диапазон**.

**Объёмная формула** (для кабелей и т.п.): `Qty * Vol * 1.2` (кабели), `*1.1` (прочее). Формулу пользователю не показывать, только результат.

### 9.8 Clarification on funnel — уточнения только по реальной воронке

Когда боту нужно сузить воронку (либо `total > full_scan_limit` в §9.7, либо стандартный clarification §11.2a при `resolved < 2`), вопрос строится **только из реальной воронки**, а не из общего справочника категории.

1. Тянем образец воронки: первые `app_settings.clarification.funnel_sample_size` товаров (default 50). Это та же первая страница, что и в §9.7 шаг 1 — повторный запрос не делается.
2. Строим распределение по `product.options[]`: для каждого `key` собираем мультимножество `value_ru` среди этих товаров.
3. Ищем «делящий» фасет: тот, у которого ≥2 различных значения, каждое с `count ≥ 1`, и распределение ближе к равномерному (по упрощённой метрике entropy / max-share). Игнорируем фасеты, у которых одно значение покрывает >90% выборки (они не сузят воронку).
4. Берём топ-`app_settings.clarification.max_values_in_question` (default 6) значений по убыванию `count`. Остальные — в опцию «другое».
5. Composer формулирует один короткий вопрос: «Уточните, пожалуйста, *{caption_ru}*: *{v1}*, *{v2}*, *{v3}*, …» **Без** показа `count` рядом со значениями.
6. **Гарантия**: каждое предложенное значение имеет `count ≥ 1` в текущей воронке. После ответа пользователя итоговый `total` для выбранного значения **не может быть 0** — иначе defect `clarification_zero_result` (§4.5).

Запрещено брать значения из `/categories/options` для clarification, не сверившись с фактом в текущей под-воронке. Если в выборке ни один фасет не делит её адекватно — clarification невозможен; см. §9.7 шаг 4.

### 9.9 Замены и аналоги
...
### 9C.1 Сортировка по цене (`sort=` игнорируется) — Scan-or-Clarify

API не поддерживает сортировку. Реализуем локально по правилам §9.7 (Scan-or-Clarify):

- `total ≤ app_settings.price_sort.full_scan_limit` (default 300) → полный честный скан всех страниц параллельно (≤6 запросов, concurrency ≤4), фильтр `price > 0`, локальный sort, топ-N.
- `total > 300` → товары **не отдаём**, задаём один уточняющий вопрос по фасету из реальной воронки (§9.8). После ответа пользователя — снова смотрим `total`.
- Если уточняющих фасетов больше нет, а `total > 300` — частичный скан первых 300 + явное предупреждение «показал самые дешёвые из 300 отсканированных, всего {total} — сузьте по бренду/типу для точного минимума».
- При желании пользователя «до X тенге» / «от X» — используем `min_price`/`max_price` (работают), это всегда предпочтительнее локального сорта. **Никогда** не подставляем `min_price`/`max_price`, не названные пользователем (см. §4.5).

Метрики: `price_sort_full_scan_total`, `price_sort_clarification_required_total`, `price_sort_partial_scan_with_warning_total`, `price_sort_pages_fetched` (histogram), `funnel_narrowing_rounds` (histogram), `auto_narrowing_attempts_total` (=0 strict), `clarification_zero_result_total` (=0 strict).

### 9C.2 Recovery-then-degrade при «молчаливом нуле» от API

**Зафиксированный дефект (аудит 28.04.2026).** `options[<key>][]=<value>` у 220volt API корректно фильтрует только когда `<key>` состоит полностью из ASCII-символов (`brend__brend` ✅, числовые значения через любой ключ ✅). Любой ключ с не-ASCII символами (`cvet__tүs`, `edinica_izmereniya__Өlsheu_bіrlіgі`, `forma_kolby__kolbanyң_pіshіnі`) при ЛЮБОМ строковом значении молча отдаёт `total=0`, хотя `products_count` в фасете > 600. Альтернативный синтаксис `options[][value]` из swagger тоже не работает. Дефект серверный, обходить кодированием/нормализацией бесполезно.

**Запрещено** вести параллельный реестр «рабочих» / «битых» ключей (whitelist, probe-cron, ручные таблицы) и подмешивать значения фасетов в `?query=`. Когда 220volt починит сериализацию — обходной путь должен отключиться сам, без релиза.

**Архитектурный принцип.** Справочник характеристик категории и сами карточки товаров содержат всю нужную информацию: каждый `ProductResource.options[]` хранит честные `key` + `value_ru` каждой характеристики товара. Если API-фильтр по характеристике врёт — ту же характеристику можно проверить локально по уже полученным товарам, **без хардкода под конкретное значение или ключ**. Это и есть recovery-этап.

**Поведение Catalog Search — recovery-then-degrade по факту ответа API:**

1. **Direct attempt.** Все резолвленные Facet Matcher'ом фильтры **всегда** сначала отправляются как `options[<key>][]=<value>`, как описывает контракт API. Никаких предположений о «рабочести» ключа заранее не делается.
2. **Если `total > 0`** — обычный happy-path, recovery и degrade не запускаются.
3. **Recovery-step (триггер: `total === 0` ∧ среди `slot.applied_filters` есть ≥1 фильтр со строковым `value` — то есть value НЕ распознан как numeric/boolean из `values[]` соответствующего facet'а).** Catalog Search:
   1. Повторяет запрос к `/products`, оставляя **только**: `pagetitle` (категория), `min_price`/`max_price`, `query` (если он был задан пользователем), и все `options[]`-фильтры с **числовыми/boolean** значениями (они работают через любой ключ).
   2. Берёт расширенное окно кандидатов: `per_page=50`, до `recovery_max_pages=5` страниц подряд, пока не наберётся `≥ recovery_min_candidates=200` товаров или не закончится pagination. Лимиты — параметры `app_settings.recovery.*`, не хардкод.
   3. Локально оставляет товары, у которых для **каждого** ранее сброшенного строкового фильтра нашлась запись в `product.options[]` с тем же `key` и `value_ru`, нормализованным по тем же правилам §9.2 (lower, NFC, trim). Сравнение строгое по `key`+`value_ru`; никаких слов из `query` пользователя в фильтр не подмешивается.
   4. Если после локального фильтра остаётся `≥1` товар — Catalog Search возвращает их как **полноценный точный результат** (с пометкой `slot.recovered_filters: AppliedFilter[]` для метрик, **без** предупреждения пользователю — фильтр сработал, просто другим путём). Composer ведёт себя ровно как при happy-path.
4. **Degrade-step (только если recovery-step невозможен или вернул 0).** Сбрасывается последний добавленный `slot.applied_filters`, фильтр уходит в `slot.degraded_filters` с `reason: 'api_returned_zero_recovery_failed'`, запрос повторяется. Максимум 2 degrade-step'а на turn. Composer выводит отдельной строкой: «Не удалось точно отфильтровать по «{caption}: {value}» — показал результаты без этого условия».
5. Если после двух degrade-step'ов всё ещё `total === 0` — Soft 404 (§11.2a).
6. Если 220volt починят сериализацию — `total > 0` уже на шаге 1, и recovery/degrade не сработают вовсе. Никаких изменений в коде/конфиге для этого не нужно.

**Что НЕ делает recovery-step:**
- не подменяет пользовательский `?query=` значениями фасетов;
- не ведёт persisted-реестр «битых» ключей;
- не обходит дефект для конкретного слова — алгоритм одинаков для «чёрный», «графитовый», «капсула», «E27» и любого другого строкового value;
- не смягчает сравнение (никаких «похожих» цветов), сравнение строгое по `key`+`value_ru`.

**Метрики:** `facet_recovery_triggered_total` (counter), `facet_recovery_succeeded_total{key}` (counter — recovery вернул ≥1 товар), `facet_recovery_pages_fetched` (histogram), `facet_degrade_step_count` (histogram 0..2 — теперь срабатывает значительно реже), `facet_degraded_total{key}` (counter — диагностика, какие ключи не лечатся даже recovery), `facet_degrade_exhausted` (counter — Soft 404 после 2 шагов).
...
### 9C.3 Товары с `price = 0` — жёсткий запрет вывода

В категории «Лампы» **50 % товаров имеют `price=0`** (архив / «цена по запросу» / снятые с продажи). Аналогичная картина в других категориях.

**Жёсткое правило (без исключений и без пользовательских настроек).** Товары с `price === 0` **никогда** не показываются пользователю — ни в основной выдаче, ни в Soft Fallback, ни в cross-sell, ни в replacement, ни в карточке артикула, ни в «похожих». Любой выходной список Composer'а **обязан** быть отфильтрован по `price > 0` непосредственно перед рендером — даже если фильтрация уже произошла на уровне Catalog Search (двойная защита от регрессий).

**Поведение Catalog Search:**
- На входе в Composer все `results[]` имеют `price > 0`. Это инвариант, нарушение — defect категории `zero_price_leak` (§22).
- Если после фильтрации `results.length === 0` и общий `total > 0` — Composer **не показывает товары вовсе** и выдаёт `CONTACT_MANAGER` с короткой строкой «По вашему запросу есть позиции, но их цена сейчас уточняется. Свяжитесь с менеджером — подскажет актуальную стоимость и наличие.»
- Если после фильтрации недобор (`results.length < requested_limit`) — Catalog Search догружает следующие страницы (до `app_settings.zero_price_max_pages`, default 5) пока не наберёт лимит или не упрётся в `pagination.pages`.

Конфиг: `app_settings.zero_price_max_pages` (integer, default 5). **Снят** конфиг `zero_price_policy` — режимы `show_with_marker`/`allow` отвергнуты архитектурно (ADR 28.18 пересмотрен 28.04.2026: «под заказ»/0₸ не должны попадать пользователю никогда). Метрика: `zero_price_hidden` (counter), `zero_price_leak` (counter — должна быть строго 0).
...
|---|---|---|
| `pending_clarification` присутствует И `applied_filters.length < 2` (Soft Fallback недоступен) | **Поиск не выполнялся.** Composer задаёт **один короткий вопрос** в формате: «Уточните, пожалуйста: какой *{facet_caption}*?» Список доступных значений добавляется **только** если их ≤6 и они короткие; иначе вопрос идёт без перечисления. **Запрещено** показывать товары и любые другие списки. `branch_payload` игнорируется. | Только этот вопрос. |
| `pending_clarification` присутствует И `applied_filters.length ≥ 2` (**Soft Fallback**) | **Поиск выполнен** по `applied_filters` (без неразрешённого трейта). Composer показывает товары + **одну короткую** non-blocking строку **в конце**: «Если важно уточнить *{facet_caption}* — напишите.» **Без** перечисления отсутствующих значений и **без** фразы «не нашёл, показал по остальным». Слот переходит в `SLOT_OPEN`/`SLOT_REFINING`; `pending_clarification` сохраняется в trace для аналитики. | Сначала список товаров, затем короткая строка-предложение. |
| `applied_aliases.length > 0` (Lexicon сработал) | **По умолчанию ничего не выводить.** Строка «Учёл синонимы» добавляется **только** если `confidence < 0.85` ИЛИ выдача 0 товаров — иначе пользователю это лишний шум. | Перед списком товаров (если выводится). |
| `category_uncertain=true` (без `pending_clarification`) | Перед списком товаров короткая строка: «Ищу в категории *{category_hint}*. Если другая — уточните.» | Первая строка ответа. |
| `soft_matches.length > 0` | **По умолчанию ничего не выводить.** Composer показывает товары как есть. Строка про soft-match добавляется **только** если в выдаче ≤2 товаров. Формат: «Ближайшее по *{trait}*: *{suggested_value}*.» Объединять однотипные. | Над списком товаров (если выводится). |
| `unresolved_traits.length > 0` (без `pending_clarification`) | **Не выводить отдельной строкой.** Информация остаётся в trace для аналитики. Если её учёт критичен — сработает Soft Fallback и Composer добавит свою короткую строку (см. выше). | — |
...
**Порядок секций финального ответа** (фиксируется здесь, нарушение — defect `composer_contract_violation`):

1. (опц.) Короткий ведущий комментарий (1 строка): `category_uncertain` подсказка ИЛИ Lexicon-нота при `confidence<0.85`.
2. Основной контент: список карточек (для `product_search/refine_filter`) ИЛИ similar-карточки (для `similar_search`, см. §11.6) ИЛИ один уточняющий вопрос (blocking-clarification / similar-clarify / price-sort-clarify) ИЛИ Soft-404.
3. (опц.) **Cross-sell-абзац** (см. §11.5) — только когда основной контент = карточки `product_search/refine_filter` И `1 ≤ results.length ≤ 3`. **Не выводится** для `similar_search`, blocking-clarification, Soft-404, Soft Fallback-tail-only.
4. (опц.) Tail-строка: Soft-Fallback «Если важно уточнить *{facet}* — напишите.» ИЛИ similar-tail «Это единственный аналог по *{critical-traits}*» / «Точные характеристики не указаны — это все товары в категории; уточните, что важнее сохранить.»

Cross-sell и similar-tail взаимно исключаются (similar никогда не сопровождается cross-sell — двойной апселл-шум).

### 11.5 Cross-sell — текстовый абзац-предложение в конце выдачи

**Что это.** 1–3 коротких предложения живым стилем продавца **после** карточек основной выдачи. Это продолжение разговора, **не интерактив** — никаких ссылок, кнопок, новых intent'ов, состояния. Не путать с §17.4 (`related_sku`/PDF — это другое).

**Когда выводится.** Все условия одновременно:
- `intent ∈ {product_search, refine_filter}`;
- `1 ≤ results.length ≤ 3`;
- не активен blocking clarification;
- не Soft-404;
- не similar-flow (`intent ≠ similar_search`);
- не Soft Fallback в режиме «tail-only без карточек».

При `results.length > 3` или `similar_search` — cross-sell **не выводится** (это не дефект; метрика `crosssell_skipped_due_to_length_total{bucket}` фиксирует пропуски для будущей калибровки порога).

**Как генерируется.** Часть основного Composer-вывода в одном LLM-вызове (не отдельный запрос — нет дополнительной latency). В системном промте Composer'а — отдельный раздел с правилами ниже.
...
| Условие | Действие |
|---|---|
| Содержит markdown-ссылку `[…](http…)` | Абзац вырезается целиком, `crosssell_invariant_violation_total++` |
| Содержит число рядом с `₸` / `руб` / `KZT` | То же |
| Содержит «нажмите/перейдите/по ссылке/кликните» (case-insensitive) | То же |
| Длина >3 предложений (`split('. ').length > 3`) | Усечение до 3, `crosssell_truncated_total++` |

**Инварианты:**
- **CS1.** Только при `1 ≤ results.length ≤ 3` для `product_search/refine_filter`.
- **CS2.** Только текст. Никаких ссылок, маркдаун-списков, карточек.
- **CS3.** Не упоминает конкретные SKU, цены, бренды, склады.
- **CS4.** Не предлагает ту же категорию, что в выдаче (категория из `slot.category_pagetitle` запрещена явным негативом в промте).
- **CS5.** 1–3 предложения, один абзац (`crosssell_invariant_violation_total === 0` и `crosssell_truncated_total ≤ 5%`).
- **CS6.** Не выводится для `similar_search` (взаимоисключение с similar-tail; см. §11.2a).

### 11.6 Similar / Аналоги — по запросу через LLM-расщепление характеристик

**Триггер.** `intent = similar_search` (см. §7.2). Маркеры: «похожее», «аналог», «замена», «такое же, но», «не хуже», «получше», «вместо этого».

**Якорь** (`anchor: Product`) — выбирается в порядке приоритета:
1. Явный артикул/ссылка в текущей реплике пользователя (regex `^\s*[A-ZА-Я0-9\-]{4,}\s*$` или URL `/p/...`);
2. `slot.anchor_product` (установлен ранее в диалоге);
...
Если якорь не найден ни одним способом → **SIM1**: бот задаёт **один** уточняющий вопрос «На какой именно товар подобрать аналоги? Пришлите артикул или скажите "на первый из списка".» Никаких догадок. Метрика `similar_no_anchor_clarification_total++`.

**Алгоритм.**

1. **Расщепление якоря.** Берём `anchor.category` и `anchor.options[]` из API (уже структурированные данные).

2. **LLM-классификация трейтов** через **structured tool calling** (§11.6a):
   - tool `classify_traits({ critical: [{key, value}], flavor: [{key, value}] })`;
   - `tool_choice: { type: "function", function: { name: "classify_traits" } }` — принудительный вызов;
   - параметр `key` в tool-схеме — **enum из `anchor.options[].key`** (закрывает галлюцинации на уровне SDK; LLM не может выдумать несуществующий ключ);
   - `critical[]` — характеристики класса совместимости (для лампы — цоколь; для розетки — номинальный ток + тип монтажа; для кабеля — сечение + число жил);
   - `flavor[]` — вкусовые (бренд, цвет, цветовая температура, форма колбы);
   - модель: `google/gemini-2.5-flash`, `temperature=0`;
   - бюджет latency: +600ms в критическом пути similar-flow (приемлемо для явного запроса аналога).

3. **Critical fallback (§11.6b).** Если LLM вернул `critical = []`:
   - запросить `category_total` через первую страницу `?category={anchor.category.pagetitle}&per_page=1`;
   - если `category_total > 20` → один уточняющий вопрос «По каким характеристикам подобрать аналог? Например: цоколь, мощность, бренд» (примеры берутся из реальных `anchor.options[].caption_ru`, не выдумываются). Ответ парсится Facet Matcher'ом → становится `critical[]`. Метрика `similar_critical_fallback_total{branch="clarify_question"}++`;
   - если `category_total ≤ 20` → показываем все товары категории как «аналоги» с tail «Точные характеристики не указаны — это все товары в категории; уточните, что важнее сохранить.» Метрика `similar_critical_fallback_total{branch="show_all_small_category"}++`.

4. **Catalog API запрос** с фильтрами по `critical[]` через тот же Strict Search и Recovery-then-Degrade (§9C.2) для non-ASCII facet keys. Кандидаты:
   - исключаем сам якорь (`product.id !== anchor.id`);
   - исключаем `price = 0` (§9C.3);
   - не применяем никаких прокси-фильтров «цена ≥ 70%» — отвергнуто архитектурным ревью.

5. **Funnel-cap (симметрично §9.7 Scan-or-Clarify):** пусть `N = candidates.length`.

   | N | Действие | Метрика |
   |---|---|---|
   | 0 | Honest message: «Прямых аналогов с *{critical-trait}* не нашёл. Если важно сохранить *{trait}* — могу подобрать у других брендов.» Карточки не выводятся. | `similar_zero_results_total++`, `similar_funnel_size_bucket{0}++` |
   | 1 | Одна карточка + tail «Это единственный аналог по *{critical-traits}*.» | `similar_funnel_size_bucket{1}++` |
   | 2-5 | Локально сортируем по числу совпавших `flavor[]`-трейтов с якорем (descending), **рендерим топ-3** (`render_top=3`). Это **не self-narrowing** — funnel сужен пользователем критичными трейтами; локальный ранкинг по совпадениям — стандартная сортировка, не фильтр. | `similar_funnel_size_bucket{2-5}++` |
   | 6+ | Один уточняющий вопрос по `flavor[]`-трейту из **реальной выдачи** (§9.8: значения с `count ≥ 1` в текущем funnel; никогда из `/categories/options` без сверки). Карточки не выводятся до ответа пользователя. | `similar_funnel_size_bucket{6+}++`, `similar_clarification_total++` |

6. **«Не хуже» → диалог, не прокси-фильтр (§11.6c).** Если в реплике пользователя обнаружен `quality_qualifier ∈ {not_worse, better}` И есть якорь:
   - **перед** шагом 4 задаём **один** уточняющий вопрос: «Что важнее сохранить: бренд того же уровня, цена не ниже исходной, или конкретные характеристики?»;
   - ответ пользователя парсится → дополнительный фильтр накладывается поверх `critical[]` через тот же Facet Matcher pipeline;
...
- **SIM2.** Кандидаты только из `anchor.category.id`. Выход за категорию запрещён.
- **SIM3.** `critical[]` определяется LLM на лету по `anchor.options[]` через structured tool calling. **Никаких статичных карт** в `app_settings`.
- **SIM4.** `funnel_cap = 5`, `render_top = 3`. ≥6 → один уточняющий вопрос по flavor из реальной выдачи.
...
- **SIM8.** `auto_narrowing_attempts_total += 0` (то же правило, что для основного поиска §9.7).
- **SIM9.** Cross-sell-абзац (§11.5) для similar-flow **не выводится** (взаимоисключение).

### 11.3 Параметры LLM

- Модель: `google/gemini-2.5-flash`.
- `temperature=0.3`, `top_p=0.9`.
- System prompt содержит: персону, формат markdown, запрет приветствий, правила эскалации, **обязательные блоки §11.2a**, **правила cross-sell §11.5/§11.5a/§11.5b**, **правила similar §11.6/§11.6a/§11.6b/§11.6c**.
- `max_output_tokens=1200`.
- Дополнительный structured-вызов `classify_traits` (§11.6a) — `temperature=0`, tool-choice forced, параллельно с Catalog Search не запускается (нужен результат до запроса).
...
- Если у товара есть `files` (PDF паспорта/инструкции) — предложить контекстно: «Паспорт изделия: [ссылка]».
- Если у товара есть `related_sku` (поле API `soputstvuyuschiy`) — допустимо упомянуть одной фразой типа «К этой розетке подходят рамки: [ссылка]». Сопутствующие SKU фильтруются по `price > 0` (§9C.3).
- Никогда не выдумывать поля — только если они присутствуют в ответе API.

> **Важно:** «cross-sell как абзац-предложение в конце выдачи» описан **отдельно** в §11.5 — это **не** про конкретные сопутствующие SKU, а про общий апселл-комментарий в стиле живого продавца. Аналоги по запросу пользователя — §11.6 (формат вывода — §17.7).
...
- **Формат строки «Наличие»:**
  - Если есть склад в городе пользователя с `amount > 0`:
    `Наличие: В наличии — *{user_city} {amount} шт*` + остальные склады через запятую (до 3 штук, остальные сворачиваются в «и ещё N городов»).
  - Если город пользователя известен, но в нём `amount = 0` или склада нет, при этом есть другие склады:
    `Наличие: В вашем городе ({user_city}) нет на складе. Ближайший: *{nearest_city} {amount} шт*` + до 2 ещё через запятую.
  - Если город пользователя неизвестен и есть склады с `amount > 0`:
    `Наличие: В наличии — *{city1} {amount1} шт*, *{city2} {amount2} шт*` (до 3 городов).
  - Если все склады с `amount = 0` ИЛИ `warehouses` пуст/отсутствует — строка «Наличие» **не выводится вовсе** (пустые поля никогда не показываем; «Под заказ» больше не используется — товар без остатков остаётся в выдаче только при `price > 0`, иначе он отфильтрован §9C.3).
...
### 17.7 Аналоги — формат вывода (similar)
...
- Cross-sell-абзац (§11.5) ≤ 3 предложений, один абзац.
...
| `facet_recovery_triggered_total` | counter | сколько раз запускался recovery-step §9C.2 (direct API дал ложный 0 при наличии строкового фильтра) |
| `facet_recovery_succeeded_total` | counter (split by `key`) | сколько раз recovery вернул ≥1 точное совпадение по `product.options[]` — главный показатель восстановления качества |
| `facet_recovery_pages_fetched` | histogram 1..5 | сколько страниц кандидатов потребовалось recovery-step'у (диагностика стоимости) |
| `facet_degrade_step_count` | histogram 0..2 | сколько degrade-step'ов потребовалось на turn (§9C.2; срабатывает только когда recovery невозможен/пуст) |
| `facet_degraded_total` | counter (split by `key`) | сколько раз каждый ключ был сброшен degrade-step'ом (диагностика «битых» ключей API, не вылеченных recovery) |
...
| `price_sort_full_scan_total` | counter | честный полный скан в §9.7 (`total ≤ full_scan_limit`) — главный показатель корректности «дешёвле/дороже» |
| `price_sort_clarification_required_total` | counter | сколько раз §9.7 ушёл в clarification-mode (`total > full_scan_limit`) |
| `price_sort_partial_scan_with_warning_total` | counter | частичный скан с предупреждением (раунды уточнений исчерпаны, делящих фасетов не осталось) |
| `price_sort_pages_fetched` | histogram 1..6 | фактическое число страниц параллельного скана |
...
| `zero_price_max_pages` | integer | макс. страниц догрузки при отбраковке `price=0` (см. §9C.3). Дефолт `5`. **Removed:** `zero_price_policy` — товары с `price=0` теперь скрываются всегда, без режимов (ADR 28.18 пересмотрен 28.04.2026). |
| `price_sort` | jsonb | параметры Scan-or-Clarify (§9.7, §9C.1). Дефолт: `{ "full_scan_limit": 300, "max_pages_parallel": 6, "parallel_request_concurrency": 4 }`. Изменение `full_scan_limit` — баланс «честность ответа vs latency»; повышать только после анализа `price_sort_pages_fetched`. |
| `clarification` | jsonb | параметры построения уточняющих вопросов из реальной воронки (§9.8). Дефолт: `{ "funnel_sample_size": 50, "max_values_in_question": 6, "min_count_per_value": 1, "max_dominance_ratio": 0.9 }`. |
