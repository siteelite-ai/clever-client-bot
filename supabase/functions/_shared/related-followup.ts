...
Определи НАМЕРЕНИЕ клиента (по смыслу, а не по словарю фраз):
- "accept": клиент выражает согласие или интерес увидеть именно то, что предложил бот. Никаких новых сущностей/параметров/тем — только реакция на предложение бота.
- "decline": клиент отказывается от предложения бота без введения новой темы.
- "new_request": клиент вводит новый запрос, новую сущность, новый параметр, оффтоп или меняет фильтры — даже если начинает с согласия/отказа.
- "unclear": намерение определить нельзя.`;
