...
serve(async (req) => {
  ...
  try {
    ...
    // Загружаем контакты НАПРЯМУЮ по заголовку (не через полнотекстовый поиск!)
    const contactsPromise = (async () => {
      ...
      try {
        ...
        const { data } = await sb.from('knowledge_entries').select('content').ilike('title', '%контакт%режим%').limit(1).single();
        return data?.content || '';
      } catch { return ''; }
    })();
...
Если у филиала нет телефона/режима в данных — просто пропусти эти строки (не пиши "нет информации"). Единые контакты: WhatsApp и Email (указаны в данных филиалов).

# Эскалация менеджеру
Когда нужен менеджер — добавь маркер [CONTACT_MANAGER] в конец сообщения (он скрыт от клиента, заменяется карточкой контактов). Сам контакты не перечисляй.
