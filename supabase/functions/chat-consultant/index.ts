...
- НЕ СОЗДАВАЙ ссылки на несуществующие разделы типа https://220volt.kz/catalog/perforatory/`;
    ...
    if (detectedCity && userCountryCode === 'KZ') {
      ...
      geoContext = `ГЕОЛОКАЦИЯ: Клиент из России${detectedCity ? ` (город: ${detectedCity})` : ''}. Когда речь о филиалах — скажи: "Я вижу, что вы из России${detectedCity ? ` (${detectedCity})` : ''}! Наши магазины расположены в Казахстане. Подскажите, какой город в Казахстане вас интересует? Мы представлены в Алматы, Астане, Актобе, Шымкенте и Караганде." Предложи помощь с доставкой.`;
    } else if (isVPN) {
      geoContext = `ГЕОЛОКАЦИЯ: Город не удалось определить (возможно VPN). Если клиент спрашивает про филиалы — скажи: "К сожалению, не удалось определить ваш город автоматически. Подскажите, какой город вас интересует?"`;
    } else {
