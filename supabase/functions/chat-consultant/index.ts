...
// JARGON FALLBACK
// ----------------------------------------------------------------------------
// Triggered when total=0 after primary search. Two-stage:
//   1) Lookup user query (normalized) in `jargon_lexicon` table → if hit,
//      retry search with `canonical` term, increment `hits`/`last_used_at`.
//   2) If no lexicon match → ask LLM for up to 3 alternative terms
//      (translations, common synonyms, technical names) → retry each in
//      sequence, return first non-empty result.
// All outcomes logged via console.log for analytics. NO auto-write to lexicon.
// ============================================================================
type JargonFallbackResult = {
  products: Product[];
...
function _normalizeJargonTerm(s: string): string {
  return s.toLowerCase().replace(/[^a-zа-яё0-9\s-]/gi, ' ').replace(/\s+/g, ' ').trim();
}

async function _lookupLexicon(term: string): Promise<{ canonical: string; id: string } | null> {
  const SUPABASE_URL = Deno.env.get('SUPABASE_URL');
  const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
  ...
  try {
    ...
    const { data, error } = await sb
      .from('jargon_lexicon')
      .select('id, term, canonical')
      .ilike('term', term)
      .limit(1)
      .maybeSingle();
    if (error || !data) return null;
    return { canonical: data.canonical, id: data.id };
  } catch (e) {
    console.error('[JargonFallback] lexicon lookup error:', e);
    return null;
  }
}

async function _bumpLexiconHit(id: string): Promise<void> {
  const SUPABASE_URL = Deno.env.get('SUPABASE_URL');
  const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
  ...
  (async () => {
    try {
      const sb = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
      // increment via select-then-update (no RPC required)
      const { data } = await sb.from('jargon_lexicon').select('hits').eq('id', id).maybeSingle();
      const next = (data?.hits || 0) + 1;
      await sb.from('jargon_lexicon').update({ hits: next, last_used_at: new Date().toISOString() }).eq('id', id);
    } catch (e) {
      console.error('[JargonFallback] hit bump error:', e);
    }
  })();
}

async function _askLLMAlternatives(term: string, settings: CachedSettings | null): Promise<string[]> {
  try {
    if (!settings?.openrouter_api_key) {
      console.log('[JargonFallback] no OpenRouter key, skipping LLM');
      return [];
    }
    const model = (settings.classifier_model || 'gemini-2.5-flash-lite').includes('/')
      ? settings.classifier_model!
      : `google/${settings.classifier_model || 'gemini-2.5-flash-lite'}`;
    ...
    const body = {
      ...
      messages: [
        {
          role: 'system',
          content: `Ты помощник поиска по каталогу электротоваров.
Пользователь задал запрос с возможным бытовым/жаргонным названием товара, по которому в каталоге ничего не нашлось.
Твоя задача: предложить до 3 альтернативных терминов для повторного поиска.

Что учитывать:
- Английские эквиваленты (кукуруза → corn; бочонок → barrel)
- Технические названия (груша → лампа A60; свеча → лампа C37)
- Популярные синонимы

Ответь СТРОГО JSON-массивом строк, без пояснений. От 1 до 3 элементов.
Пример: ["corn", "лампа кукуруза LED"]
Если не уверен — верни пустой массив [].`,
        },
    ...
    const fetchPromise = callAIWithKeyFallback(
    ...
    const response = await Promise.race([fetchPromise, timeoutPromise]);
    if (!response.ok) {
      console.log(`[JargonFallback] LLM error ${response.status}`);
      return [];
    }
    const data = await response.json();
    const content = data.choices?.[0]?.message?.content || '';
    ...
    return parsed
      .filter((s: unknown) => typeof s === 'string' && (s as string).trim().length >= 2)
      .map((s: string) => s.trim())
      .slice(0, 3);
  } catch (e) {
    console.error('[JargonFallback] LLM error:', e);
    return [];
  }
}

async function tryJargonFallback(
  userMessage: string,
  candidates: SearchCandidate[],
  limit: number,
  settings: CachedSettings | null,
): Promise<JargonFallbackResult> {
  const term = _normalizeJargonTerm(userMessage);
  const baseCandidate = candidates[0];
  if (!term || !baseCandidate) {
    return { products: [], source: 'none', term, canonical: null };
  }

  // 1) Lexicon lookup
  const hit = await _lookupLexicon(term);
  if (hit) {
    const altCandidate: SearchCandidate = {
      ...baseCandidate,
      query: hit.canonical,
      category: null,
    };
    const products = await searchProductsMulti([altCandidate], limit, settings?.volt220_api_token || undefined);
    console.log(`[JargonFallback] event=jargon_fallback source=lexicon term="${term}" canonical="${hit.canonical}" hit=${products.length > 0}`);
    if (products.length > 0) {
      _bumpLexiconHit(hit.id);
      return { products, source: 'lexicon', term, canonical: hit.canonical };
    }
  }

  // 2) LLM alternatives
  const alternatives = await _askLLMAlternatives(term, settings);
  console.log(`[JargonFallback] event=jargon_fallback source=llm term="${term}" alternatives=${JSON.stringify(alternatives)}`);
  for (const alt of alternatives) {
    ...
    const products = await searchProductsMulti([altCandidate], limit, settings?.volt220_api_token || undefined);
    if (products.length > 0) {
      console.log(`[JargonFallback] event=jargon_fallback source=llm term="${term}" canonical="${alt}" hit=true`);
      return { products, source: 'llm', term, canonical: alt };
    }
  }
