...
/**
 * Search products by EXACT pagetitle parameter (Catalog API: ?pagetitle=...).
 * Mirrors searchByArticle: bounded fetch with retry, drops price=0 (HARD BAN).
 * Returns at most 5 results.
 */
async function searchByPagetitle(title: string, apiToken: string): Promise<Product[]> {
  const params = new URLSearchParams();
  params.append('pagetitle', title);
  params.append('per_page', '5');

  console.log(`[TitleSearch] Searching by pagetitle: "${title}"`);
  ...
  try {
    const rawData = await response.json();
    const data = rawData.data || rawData;
    const results: Product[] = (data.results || []).filter(
      (p: Product) => typeof p?.price === 'number' && p.price > 0
    );
    console.log(`[TitleSearch] Found ${results.length} product(s) for pagetitle "${title}"`);
    return results;
  } catch (error) {
    console.error(`[TitleSearch] Parse error:`, error);
    return [];
  }
}
...
8. ЦВЕТ vs ЦВЕТОВАЯ ТЕМПЕРАТУРА: в схеме могут одновременно присутствовать ДВА «цветовых» ключа. Ключ с caption "Цвет" — это цвет корпуса/материала (чёрный, белый, графит, медь). Ключ с caption "Цветовая температура" — это оттенок света для ламп/светильников (тёплый, нейтральный, холодный, 2700K, 4000K, 6500K). Выбирай ключ строго по физическому смыслу модификатора: «чёрный/белый/графит» → ключ "Цвет"; «тёплый/холодный/нейтральный/NNNNK» → ключ "Цветовая температура". Если модификатор «белый свет» или явно про оттенок света — это температура, а не цвет корпуса.
