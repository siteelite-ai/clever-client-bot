...
Аудит 28.04 показал: часть фасет-ключей API (например, `cvet__tүs`, `forma_kolby__kolbanyң_pіshіnі`) возвращают `total = 0` даже при значениях, физически присутствующих в схеме. Закономерность — **per-key**, не сводится к «есть казахская буква». **Любая попытка вести параллельный реестр «рабочих» / «битых» ключей (whitelist, probe-cron, ручные таблицы) запрещена** — это хардкод-патч, протухающий при первом изменении API.
...
2. Если ответ API имеет `total === 0` **И** в запросе было ≥ 1 `options[]`-фильтра — выполняется **degrade-step**: запрос повторяется без последнего по приоритету `options[]`-фильтра. Приоритет — порядок добавления в `slot.applied_filters` (последний добавленный сбрасывается первым, как наименее «опорный» для пользователя).
3. Шагов деградации **не более 2**. Каждый сброшенный фильтр добавляется в `slot.degraded_filters: AppliedFilter[]` с полем `reason: 'api_returned_zero'` и выводится Composer'ом отдельной строкой: «Не удалось точно отфильтровать по «{caption}: {value}» — показал результаты без этого условия».
