...
ПРАВИЛА:
- Две части: (1) короткая констатация про сопутствующие категории, (2) короткий вопрос-предложение подобрать. Всего до 240 символов.
- Часть 1: начни с естественного оборота — «С этим часто берут …», «К этому обычно докупают …», «Также пригодятся …», «Под такие розетки обычно нужны …» и т.п. — выбери под контекст вопроса клиента.
...
- Часть 2: ОДИН короткий вежливый вопрос с предложением помочь подобрать — например «Подобрать подходящие?», «Показать варианты?», «Подскажу, если нужно — подобрать?». Варьируй формулировку под контекст, без шаблонности.
...
Определи НАМЕРЕНИЕ клиента, а не буквальное совпадение слов:
- "accept": клиент ВЫРАЖАЕТ СОГЛАСИЕ или ИНТЕРЕС увидеть то, что предложил бот. Сюда входят:
  * короткие подтверждения: "да", "давай", "ок", "хорошо", "ага", "угу", "конечно", "почему бы и нет";
  * просьбы показать/вывести/прислать: "покажи", "покажите", "выведи", "пришли", "скинь", "хочу посмотреть", "интересно", "любопытно";
  * отсылки к предмету предложения (особенно если бот предложил «показать остальные / ещё»): "остальные", "ещё", "что там ещё", "давай остальные", "ну покажи", "что у вас ещё есть", "а ещё?", "продолжай", "дальше", "валяй", "го";
  * если клиент назвал ОДНУ из категорий/сущностей, упомянутых в фразе бота, — тоже accept.
  Главное правило: клиент НЕ вводит новых параметров/требований — он лишь соглашается увидеть то, что бот уже предложил. Намерение важнее формулировки.
- "decline": клиент ОТКАЗЫВАЕТСЯ БЕЗ новой темы ("нет", "не надо", "не нужно", "спасибо, не надо", "пока не надо", "достаточно", "хватит", "всё", "не сейчас", "обойдусь"). Чистый отказ — никаких новых сущностей.
- "new_request": клиент пишет ЧТО-ТО ДРУГОЕ — новый запрос, вопрос про показанные ранее товары, изменение фильтров (другой цвет/цена/бренд/длина/мощность), оффтоп. Любое сообщение с НОВОЙ сущностью или модификатором, которого не было в фразе бота. Даже если начинается с "нет" — но дальше идёт новый запрос («нет, покажи розовые», «а есть на 30 метров?») — это new_request.
