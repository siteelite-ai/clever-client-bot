...
- Базовая формула: итоговый_объём = количество × объём_единицы (из характеристик товара).
- Применяй коэффициент запаса (упаковка/намотка/паллетирование):
  • Если товар — КАБЕЛЬ или ПРОВОД (определи это сам по названию и характеристикам товара) → коэффициент **1.2**.
  • Любой другой товар → коэффициент **1.1**.
- Финальная формула: итог = количество × объём_единицы × коэффициент.
- Приведи ответ в м³. Если в характеристиках указаны дм³ / литры / см³ — переведи самостоятельно (1 м³ = 1000 л = 1000 дм³ = 1 000 000 см³) и укажи это явно.
- Пример формулировки (живым языком): «100 метров такого кабеля займут примерно 0.06 м³ с учётом запаса на намотку (коэффициент 1.2)».
