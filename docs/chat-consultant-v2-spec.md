...
|---|---|---|---|---|---|
| TC-69 | «найди чёрные двухгнёздные розетки» | `pagetitle="Розетки"`, `confidence ≥ 0.9` | `resolved=[{cvet__tүs:"чёрный"}, {kolichestvo_razyemov__aғytpalar_sany_:"2"}]`, `unresolved=[]`, `soft_matches=[{trait:"двухгнёздные",reason:"numeric_equivalent",value:"2"}]` | Catalog Search отправляет оба `options[]`. API возвращает `total=0` (известный дефект `cvet__tүs`). Запускается **recovery-step** (§9C.2): повтор запроса с числовым фильтром `kolichestvo_razyemov__aғytpalar_sany_=2` (он работает) и без `cvet__tүs`, окно до 200 кандидатов; локальный фильтр оставляет только товары, у которых в `product.options[]` есть `key="cvet__tүs"` и `value_ru="чёрный"`. Возвращается `≥1` чёрная двухгнёздная розетка. `slot.recovered_filters=[{key:"cvet__tүs",value:"чёрный"}]`, `slot.degraded_filters=[]`. Composer выводит **именно чёрные двухгнёздные** карточки **без** предупреждения о сбросе фильтра. Метрики: `facet_recovery_triggered_total++`, `facet_recovery_succeeded_total{key="cvet__tүs"}++`, `facet_degrade_step_count=0`. | §9C.2 recovery-then-degrade — точный результат через локальную проверку `product.options[]`, без хардкода под «чёрный». |
...
| TC-71 | «двойные розетки белого цвета» | `pagetitle="Розетки"`, `confidence ≥ 0.9` | `resolved=[{cvet__tүs:"белый"}, {kolichestvo_razyemov__aғytpalar_sany_:"2"}]`, `soft_matches=[{trait:"двойные",value:"2",reason:"numeric_equivalent"}]` | Аналогично TC-69: первый запрос → `total=0`, recovery-step добирает кандидатов по `kolichestvo_razyemov__aғytpalar_sany_=2` и локально оставляет товары с `cvet__tүs="белый"`. `slot.recovered_filters=[{key:"cvet__tүs",value:"белый"}]`, degrade не запускается. Composer выводит **именно белые двухгнёздные** карточки без предупреждений. | §9C.2 recovery-step + числовая нормализация «двойные→2» — точный ответ, не сброс фильтра. |
