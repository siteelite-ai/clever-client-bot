...
Если в истории разговора уже обсуждался конкретный товар или категория (например, "перфоратор", "шуруповерт", "дрель"), 
и пользователь задаёт уточняющий вопрос ("а какие бренды есть?", "какие марки?", "сколько стоит?"), 
ты ДОЛЖЕН ИСПОЛЬЗОВАТЬ категорию/товар из контекста для генерации кандидатов!

ПРИМЕРЫ С КОНТЕКСТОМ:
- Контекст: обсуждали перфораторы Bosch → "а какие бренды есть?" → query="перфоратор" (НЕ пустой запрос!)
- Контекст: обсуждали шуруповерты → "какие есть?" → query="шуруповерт"
- Контекст: искали дрели → "а дешевле есть?" → query="дрель" или "дрель недорогая"

1. Тип намерения (intent):
   - "brands" — пользователь спрашивает КАКИЕ БРЕНДЫ представлены (примеры: "какие бренды?", "какие марки?", "какие производители?")
...
- ОБЯЗАТЕЛЬНО используй категорию из контекста разговора если она есть!
- Пример: контекст="перфоратор Bosch" + "какие бренды?" → {"query": "перфоратор"}
...
- ВСЕГДА используй контекст разговора для определения категории!
...
// Заменяет тестовый домен на продакшн для пользовательских ссылок
function toProductionUrl(url: string): string {
  return url.replace(/220volt\.testdevops\.ru/g, '220volt.kz');
}
...
- НЕ ПРИДУМЫВАЙ что "в каталоге огромное количество позиций этого производителя"`;
        ...
        productInstructions = `
⚠️ ТОВАРЫ НЕ НАЙДЕНЫ В КАТАЛОГЕ ПО ЭТОМУ ЗАПРОСУ!

Поисковый запрос: "${extractedIntent.originalQuery}"

СТРОГО ЗАПРЕЩЕНО:
- НЕ ПРИДУМЫВАЙ товары, названия, цены или URL!
- НЕ СОЗДАВАЙ ссылки вида [Товар](https://220volt.kz/...) — это ГАЛЛЮЦИНАЦИЯ!
- НЕ УПОМИНАЙ конкретные модели, артикулы или цены которые ты "помнишь"

ЧТО ДЕЛАТЬ:
1. Скажи что по этому запросу нужно уточнить детали
2. Спроси: какой тип инструмента (дрель, перфоратор, болгарка)?
3. Или спроси: какой бюджет, для каких задач?
4. Предложи перейти в каталог: https://220volt.kz/catalog/`;
      }
    }
