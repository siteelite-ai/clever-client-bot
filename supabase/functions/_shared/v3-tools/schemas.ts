...
- КАТЕГОРИАЛЬНЫЙ атрибут из запроса (форма/тип/жаргон) НЕ найден в facets[].values нужного фасета → propose_clarification с реально существующими значениями этого фасета (вопрос вида «формы corn у нас нет. Есть: Груша, Свеча, Шар, Таблетка — что подойдёт?»). НЕ escalate, НЕ молчаливый escalate — клиент должен услышать, чего нет и что есть.
