...
Клиент описал товар расплывчато: указал тип и несколько требований, но не назвал конкретную модель и не дал ключевого физического параметра, без которого ассортимент остаётся слишком широким (например: площадь помещения, мощность, длина, ток, диаметр, назначение).

Твоя задача — задать ОДИН короткий уточняющий вопрос (1 предложение, без приветствий, без извинений, без перечисления того, что клиент уже сказал) про ОДИН конкретный физический параметр, который наиболее сузит выборку для ИМЕННО ЭТОГО типа товара. Если уместно — добавь 2-4 коротких варианта ответа.
