...
Deno.test("DIAG: /categories/options?pagetitle=Розетки — before/after blacklist v2 (8 keys)", async () => {
  ...
  console.log(`Blacklist size: ${FACET_BLACKLIST_KEYS.size} keys`);
  ...
  console.log(`\nFacets total: ${options.length}  → kept: ${kept.length}  (removed ${removed.length})`);
